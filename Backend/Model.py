...
AUTOMATION_RULES = [
    ...
    (r"^(set|create)\s+(a\s+)?(alarm|timer|reminder)\b", "reminder", 8),
    (r"^(cancel|delete|remove)\s+(it|that|this|them)\b", "automation", 5),
    (r"^(open|launch|start|close|kill|restart)\s+\w+", "automation", 6),
    (r"^(play|pause|resume|skip|next|previous|prev|mute|unmute)\b", "automation", 8),
    ...
    (r"^take\s+(a\s+)?screenshot\b", "automation", 8),
    (r"^screenshot\b", "automation", 8),
    ...
    (r"^generate\s+(illustration|sketch|concept art)\b", "image", 8),
    ...
    (r"^(make|create)\s+(an?|a few|some)?\s*\w*\s*(video|animation|clip|short|film|time.?lapse|trailer)\b", "video", 9),
    ...
    (r"^(how does|how do|how to|how can)\b", "general", 5),
    ...
    (r"\b(stock|share price|share|bitcoin|crypto|nifty|sensex|forex|gold rate|gold|exchange rate|live score|market|dollar)\b", "realtime", 5),
    ...
    (r"\b(volume|brightness|screen|screenshot)\b", "automation", 4),
